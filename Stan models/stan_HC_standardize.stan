data {
    ...
    matrix[n, 4] XhcS1;// the design matrix of the fixed part when we have standardized variables 
    vector[2] SDs_Xhc1;// standard deviations of the raw variables in the fixed part 
    vector[2] mean_sd_Xhc1;// means/sds of the raw variables in the fixed part 
    matrix[N1,2] Zs1;// the design matrix of the random part when we have standardized variables
    real SDs_Z1;// standard deviations of the raw variables in the random part 
    real mean_sd_Z1;// means/sds of the raw variables in the random part 
...
transformed parameters {
    ...
    matrix[n, n_RE] mu_u;
    for (i in 1:n) {
        mu_u[i, 1] = XhcS1[i, 1] * temp_betas1[1] + XhcS1[i, 3] * temp_betas1[3] + XhcS1[i, 4] * temp_betas1[4];// it will give the mean
        // of the multivariate normal distribution of the random intercept of the u 
        mu_u[i, 2] = XhcS1[i, 2] * temp_betas1[2];// it will give the mean
        // of the multivariate normal distribution of the random slope of the u
    }
    for (j1 in 1:N1) {
        eta1[j1] = Zs1[j1, 1] * u[id1[j1], 1] + Zs1[j1, 2] * u[id1[j1], 2];// estimated values 
    }
}
...
generated quantities {
    ...
    real Intercept1 = temp_betas1[1] - mean_sd_Xhc1[1] * temp_betas1[3] - mean_sd_Xhc1[2] * temp_betas1[4] - mean_sd_Z1 * temp_betas1[2];// we tranform back the intercept 
    vector[3] betas1_part; // these vector will contain the extra coefficients of the fixed part which we want to trasform back 
    ...
    // --lets transform back all the other coefficients of the fixed part 
    betas1_part[2] = temp_betas1[3] / SDs_Xhc1[1];  
    betas1_part[3] = temp_betas1[4] / SDs_Xhc1[2];
    betas1_part[1] = temp_betas1[2] / SDs_Z1;
    betas1= append_row(Intercept1, betas1_part);// it contains all the trasformed coefficients of the fixed part 
    tmp_D = diag_pre_multiply(L_var_D, L_corr_D) * diag_pre_multiply(L_var_D, L_corr_D)';
    tmp_b = u - mu_u; // these are the random terms b 

        b[, 1] = tmp_b[, 1]-tmp_b[, 2]*mean_sd_Z1; // we transform back the random intercept 
        b[, 2] = tmp_b[, 2]/SDs_Z1; // we transform back the random slope 

    D =  Zinv1 * Zs1 * tmp_D * (Zs1)' * Ztinv1;# we transform back the variance covariance matrix of the random terms 
